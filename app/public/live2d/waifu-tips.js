...
function loadWidget(waifuPath, apiPath) {
	...
	function registerEventListener() {
		...
		document.querySelector("#waifu-tool .fa-smile-o").addEventListener("click", loadOtherModel);
		document.querySelector("#waifu-tool .fa-child").addEventListener("click", loadRandModel);
		document.querySelector("#waifu-tool .fa-camera").addEventListener("click", () => {
		...
		document.querySelector("#waifu-tool .fa-times").addEventListener("click", () => {
			...
			$("#waifu").animate({ bottom: -500 }, 3000, () => {
				$("#waifu").hide();
				$("#waifu-toggle").show().animate({ "margin-left": -30 }, 1000);
			});
		});
		var re = /x/;
		console.log(re);
...
function initWidget(waifuPath = "../waifu-tips.json", apiPath = "") {
	...
	$("#waifu-toggle").hover(() => {
		...
		$("#waifu-toggle").animate({ "margin-left": -40 }, 500);
	}).click(() => {
	...
	if (localStorage.getItem("waifu-display") && Date.now() - localStorage.getItem("waifu-display") <= 86400000) {
		$("#waifu-toggle").attr("first-time", true).css({ "margin-left": -40 });
	} else {
		loadWidget(waifuPath, apiPath);
	}
}
